...
<body>
    <div class="container-fluid">
        ...
        <div class="pt-2 d-flex justify-content-between">
            ...
            <div><a class="btn btn-success py-2 align-items-center" href="./activity_comment_manage.php">คอมเมนท์ที่อนุมัติแล้ว</a></div>
	    </div>
                ...
                $sql="SELECT event_comments.id,
                   ...
                   if($result && $result->num_rows>0){  // คิวรี่ข้อมูลสำเร็จหรือไม่ และมีรายการข้อมูลหรือไม่
                       while($row = $result->fetch_assoc()) { // วนลูปแสดงรายการ
                        ...
                        <td>
                            <a href="activity_edit.php?pid=<?php echo htmlentities($row['postid']);?>">
                                <?php echo htmlentities($row['PostTitle']);?>
                            </a>
                        </td>
                        <td><?php echo date("$newDate")." ".$month_arr[date("$newMonth")]." ".(date("$newYear")+543);?></td>
                        <td><?php if($st=='0'):?>
                            <a href="activity_comment_unapprove.php?disid=<?php echo htmlentities($row['id']);?>" title="Disapprove this comment">
                                <i class="fa fa-check" style="color: #29b6f6;"></i></a>
                                <?php else :?>

                            <a href="activity_comment_unapprove.php?appid=<?php echo htmlentities($row['id']);?>" title="Approve this comment">
                                <i class="fa fa-check" style="color: #29b6f6;"></i></a>
                                <?php endif;?>&nbsp;

                            <a href="activity_comment_unapprove.php?rid=<?php echo htmlentities($row['id']);?>&&action=del">
